...
class MultitaskGridModelLightningModule(pl.LightningModule):
    def __init__(self, model, learning_rate, mask, target_variables=['pre'], 
                 trial=None, weight_decay=1e-5, lr_scheduler='step', 
...
class MultitaskConvLSTMPipeline:
    ...
    def _objective(self, trial, train_loader, val_loader, in_channels, pre_seq_len, aft_seq_len):
        ...
        if self.is_multitask:
            ...
            multitask_fusion = 'concat'
            task_weights = {self.target_variables[0]: 1.0}
        
        # Build model
        hidden_dim = [hidden_size] * n_layers
        kernel_size_tuple = (kernel_size, kernel_size)
        
        model = MultitaskEncodingForecastingConvLSTM(
...
if __name__ == "__main__":
    ...
    pipeline_single = MultitaskConvLSTMPipeline("config_single_target.yaml")
    results_single = pipeline_single.run_pipeline()
    
    # Example: Multitask
    print("\n=== Multitask Example ===")
    pipeline_multi = MultitaskConvLSTMPipeline("config_multitask.yaml")
    results_multi = pipeline_multi.run_pipeline()
    
    print("Pipeline completed successfully!")
    print("Single target results:", results_single)
    print("Multitask results:", results_multi)